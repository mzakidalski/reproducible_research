...
The EVTYPE column contains the "Summary..." rows which should be deleted. Additionaly event names are transformed to the uppercase:

```{r, echo=TRUE, warning=FALSE, message=FALSE}
head(filtered_cols_data %>% filter (str_detect(EVTYPE, "[Ss]ummary")) %>% select(EVTYPE))
filtered_cols_data <- filtered_cols_data %>% filter (!str_detect(EVTYPE, "[Ss]ummary")) %>% mutate(EVTYPE = toupper(EVTYPE))
```
